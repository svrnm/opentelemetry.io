...
2. Upgrading/downgrading agents and management of agent-specific packages,
...
4. Health and performance monitoring of the agents, typically CPU and memory
   usage and also agent-specific metrics, for example, the rate of processing or
   backpressure-related information.
5. Connection management between a control plane and the agent such as handling
...
1. The OpenTelemetry Collector, configured with pipeline(s) to:
   ...
   - (B) export signals to upstream destinations, potentially including
     ...
     connection settings).
2. The bi-directional OpAMP control flow between the control plane implementing
